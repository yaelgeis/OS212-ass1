...
syscall(void)
{
  ...
  //Added for A1T2
  //Before executing a system call, a0 stores the first argument
  ...
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    ...
    //Added for A1T2
    //After executing a system call, a0 stores the return value
    int ret = p->trapframe->a0;       

    //If this system call should be traced
    if (((1 << num) & p->mask) != 0){
      if( num == SYS_kill || num == SYS_sbrk){
        printf("%d: syscall %s %d -> %d\n", p->pid, syscall_names[num],arg, ret);
      }
